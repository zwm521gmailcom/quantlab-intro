...
本文说明 QuantLab 现在是什么、界面长什么样、怎么在本机和局域网里用，以及如何联系。本仓库公开，只放介绍、界面截图和联系二维码；源码、行情、Token 和回测产物不在这里。

| 项目 | 说明 |
|---|---|
| 产品 | QuantLab，本机量化研究与回测工作台 |
| 当前版本 | `0.4.0`（Alpha） |
| 默认资产 | A 股（数字货币请另起实例和端口） |
| 本文仓库 | 公开介绍，不含源码和数据 |
...
截自正在运行的 A 股实例 `0.4.0`。设置页含 Token 和本机路径，不放在这里。

**研究总览** — 数据集、已发布因子、模型与回测次数，以及最近运行和质量告警。

![研究总览](assets/ui/01-overview.png)

**数据中心** — 只读展示原始数据仓库（raw）和来源整理表；标准行情、因子与研究数据在因子/回测中消费。
...
**回测中心** — 从数据、因子、模型到成交规则，一次保存完整运行配置；开始回测时才训练。

![回测中心](assets/ui/06-backtest.png)

**回测计划** — 把要跑的回测写进清单。同时跑几条由本机内存和核数自动定；16GB 仍是一条一条跑。
...
- 多机研究时用拷盘或 git 同步运行库，把机器码和 Token 也带走
...
| B. 本地模块化单体 | FastAPI 提供页面和 API，SQLite 存元数据，PyArrow/Polars 读外部 Parquet，训练与回测走后台任务 | **采用**。单机和局域网研究部署简单，页面、业务、数据仓库边界清楚 |
...
| 数据中心 | `/data` | 已登记 raw / 来源整理表、覆盖范围与质量 |
...
| 回测计划 | `/backtests/plan` | 勾选任务；同时条数按内存和核数自动定 |
| 规则回测 | `/backtests/rules` | 内置规则模板，不训练模型 |
| 结果档案 | `/backtests/runs` | 历史运行、指标、产物与复制配置 |
| 设置 | `/settings` | 路径、资产与端口、Tushare、局域网、运算资源、草稿默认参数 |
...
1. **权威数据只读。** `data/` 与校准目录保持原位。研究计算写入新版本或独立运行目录。
2. **成交与因子用后复权。** 正式字段为 `hfq_open`、`hfq_high`、`hfq_low`、`hfq_close`。
3. **版本不可变。** 因子、模型、策略、数据集和回测都带稳定 ID 与版本或运行快照。历史记录不覆盖。功能更新改 `quantlab/__init__.py` 的版本号，三台机器应显示同一数字。
...
7. **失败也留档。** 配置、日志、错误和已生成文件都保留，不静默重试。系统因内存杀掉的进程不写成用户「已强行停止」。
8. **复制不等于再跑。** 从历史运行复制配置只进入草稿。自动挖掘只产生候选和研究记录，不自动发布因子。
9. **同时回测按机器算。** 没有手动档。16GB 仍是 1 条；内存更大时按内存和核数自动开路，单路内部的折/分层还会再按内存下降。
...
| 运行时 | `<project>/quantlab_runtime` | SQLite、任务、因子版本、回测结果、机器配置 |

标准行情来自 `data/canonical.parquet`（每股票每日一行）。已有数据仓库可以只改路径配置，不必搬文件。公式包因子从标准宽表落地旁路 Parquet，回测按字段挂上，不把完整因子宽表复制进网站目录。

局域网多机：

- 各台自己的 SQLite；程序更新走 GitHub（`git pull` 后重启）
- 只发现同一资产版本
- 回测产物互相补缺（已有目录不覆盖），并带上计划快照
- 数据同步勾选标准宽表 / 旁路因子 / raw / 来源整理表；先比大小和修改时间，相同的不下载
- 不同步因子库 SQLite、`quantlab_runtime/factors/`、Token 和机器码
- 可选每天 04:00 以本机为源头同步所选类别；服务须在跑，错过不补跑

**不要**用 git 或拷盘同步 `quantlab_runtime/db/`、`config/`，也不要对整个 `results/` 做镜像删除。
...
本机打开 http://127.0.0.1:8765/ 。16GB 内存机器请把分层进程数设为 `1`，不要按 CPU 核数开满。只停运算请用回测中心的「回测强行停止」，不要停整个 QuantLab。
...
- 版本 `0.4.0`，Alpha
- 目标是本机可复现的研究工作台，可在局域网多机补缺，不是托管云平台，也不是实盘交易系统
